<%@ Page Language="C#" AutoEventWireup="true" CodeFile="lista_portfolio.aspx.cs" Inherits="lista_portfolio" %>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta name="description" content="JPDado Software - Portfolio">
    <meta name="author" content="André Lourenço">
    <title>JPDado Software - Portfolio</title>
    <!-- Favicon -->
    ...
    <div class="main-content">
        ...
        <nav class="navbar navbar-top navbar-expand-md navbar-dark" id="navbar-main">
            <div class="container-fluid">
                <!-- Brand -->
                <a class="h4 mb-0 text-white text-uppercase d-none d-lg-inline-block">Portfolio</a>
                <!-- Form -->
...
<%--                        <div class="col-xl-3 col-lg-6" id="header3">
                    ...
                    <div class="card shadow">
                        <div class="card-header border-0">
                                    ...
                                    <td style="width: 15%; max-width: 15% !important; text-align: right;">
                                        <div id="divCriaNovo">
                                            <a class="btn btn-sm btn-primary" onclick="novo();" style="color: #FFFFFF;">
                                                <span id="spanBtnAlteraStatus"><i class="ni ni-building" aria-hidden="true"></i>  Criar novo portfolio</span>
                                            </a>
                                        </div>
                                    </td>
                                </tr>
                            ...
                            <form id="formUploadFile" runat="server">
                                <table style="width:100%; height: auto;">
                                    <tr>
                                        <td style="width: 25%; max-width: 25% !important; text-align: left;">
                                            <div>
                                                <button type='button' style="width:100%; height: 100%;" class='btn btn-sm btn-primary' id='buttonFileUpload' onclick='simulateClickOnFileUploadButton();' runat='server'>Carregar Ficheiro Portfolio</button>
                                            </div>                                        
                                        </td>
                                        ...
                                        <td style="width: 25%; max-width: 25% !important; text-align: center;">
                                            <div>
                                                <asp:Button runat="server" Text="Carregar Portfolio para o Sistema" ID="uploadButton" OnClick="Upload_Click" CssClass="btn btn-sm btn-primary" />
                                            </div>                                        
                                        </td>
                                ...
                                <table style="width:100%; height: auto;" class="variaveis">
                                    <tr>
                                        <td style="width: 100%; max-width: 100% !important; text-align: left;">                                       
                                            <asp:Button runat="server" Text="Carregar Imagem para o Sistema" ID="UploadImageButton" OnClick="UploadImage_Click" CssClass="btn btn-sm btn-primary" />
                                            <asp:FileUpload ID="ImageUploadControl" runat="server" CssClass="variaveis" />
                                            <asp:TextBox ID="portfolioID" runat="server" CssClass="variaveis" />
                                            <asp:TextBox ID="portfolioFlag" runat="server" CssClass="variaveis" />
                                            <p class='variaveis' id='msgReturned' runat='server'></p>
                                        </td>
                                    </tr>
                                </table>
                            </form>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $('#ImageUploadControl').change(function () {
                var path = $(this).val();
                if (path != '' && path != null) {
            ...
            var msgAfterUploadImg = $('#msgReturned').html();

            if (msgAfterUploadImg != '') {
                sweetAlertInfo("Carregamento de Imagens", msgAfterUploadImg);
                $('#msgReturned').html('');
            }
        });
        ...
        function getData() {
            loadingOn('A carregar o portfolio. Por favor aguarde...');
            var pesquisa = $('#txtPesquisa').val();
            var order = "";

            if (ordAno == 0 && ordProjeto == 0) {
                order = ' ORDER BY ano asc, tipo asc, ordem asc ';
            }
            else {
                order = ' ORDER BY ';
                ...
                else if (ordProjeto != 0) {
                    order += ordProjeto == -1 ? ' tipo desc, ordem desc, localidade desc, empresa desc ' : ' tipo asc, ordem asc, localidade asc, empresa asc ';
                }
            }

            $.ajax({
                type: "POST",
                url: "lista_portfolio.aspx/getGrelha",
        ...
        function editar(id) {
            loadUrl('config_ficha_portfolio.aspx?id=' + id);
        }

        function eliminar(id) {
            swal({
                title: 'Eliminar portfolio',
                text: "O portfolio será eliminado. Confirma?",
                type: 'question',
        ...
        function delRow(id) {
            $.ajax({
                type: "POST",
                url: "lista_portfolio.aspx/delRow",
                ...
                success: function (res) {
                    ...
                    if (parseInt(res) <= 0) {
                        sweetAlertWarning("Eliminar portfolio", resMsg);
                    }
                    else {
                        $('#txtPesquisa').val('');
                        ordAno = 0;
        ...
        function novo() {
            loadUrl('config_ficha_portfolio.aspx');
        }
        ...
        function uploadImgPortfolio(id) {
            $('#portfolioID').val(id);
            $('#portfolioFlag').val('1');
            simulateClickOnImageUploadButton();
        }

        function uploadImgCapa(id) {
            $('#portfolioID').val(id);
            $('#portfolioFlag').val('0');
            simulateClickOnImageUploadButton();
        }

        function simulateClickOnImageUploadButton() {
