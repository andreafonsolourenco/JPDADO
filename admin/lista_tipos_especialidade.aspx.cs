...
public partial class lista_tipos_especialidade : System.Web.UI.Page
{
    ...
    public static string getGrelha(string pesquisa, string order, string admin)
    {
        ...
        sql = string.Format(@"  DECLARE @id int;
                                ...
                                where (nome like {0} or ordem like {0})
            ...
            for (int j = 0; j < oDs.Tables.Count; j++)
            {
                for (int i = 0; i < oDs.Tables[j].Rows.Count; i++)
                {
                    ...
                    html += String.Format(@"    <tr class='pointer' ondblclick='editar({0});'>
                                                    ...
                                                    <td><span>{2}</span></td>
		                                            {3}                   
	                                            </tr>", id, nome, ordem, htmlOptions);
                }
            }
        }
        else
        {
            html += String.Format(@"<tr><td colspan='{0}'>Não existem tipos de especialidade a apresentar.</td></tr>", admin == "1" ? "4" : "3");
        }
    ...
    public static string delRow(string id, string idUser)
    {
        ...
        sql = string.Format(@"  DECLARE @id INT = {0};
                                ...
                                EXEC DELETE_TIPO_ESPECIALIDADE @iduser, @id, @ret OUTPUT, @retMsg OUTPUT
    ...
    protected void Upload_Click(object sender, EventArgs e)
    {
        ...
        if (FileUploadControl.HasFile)
        {
            try
            {
                ...
                string pathToSave = Server.MapPath("~") + "uploaded_files/TiposEspecialidade_" + DateTime.Now.ToShortDateString().Replace("/", "") + "_" + DateTime.Now.ToLocalTime().ToShortTimeString().Replace(":", "") + extension;
                ...
                if (oDB.insertCSVFileIntoDB(pathToSave, "TIPOS_ESPECIALIDADES", userID.Text))
                {
                    uploadFileSuccess.InnerHtml = "Tipos de Especialidade Carregados com sucesso!";
